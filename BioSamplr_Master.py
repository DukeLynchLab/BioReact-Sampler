...
def Sample(sample):
    ...
    slow_clear(20)                                   # Start with clear tubing

    slow_pump_1(10)                                   # Pump purge volume
    
    slow_clear(20)                                   # Clear tubing
    ...
    get_temp()
    print ("Sample %s Time:" % (sample), sample_time)
    with open(sample_log, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([sample, sample_time, "", Tc, ""])
	writer.writerow(["", "", "", "", ""])
...
sample_size = 20                    # 60 is current standard          

MaxTempADC = 650.6436781609         # Change this value when calibrating Thermal Probes
                                    # MaxTempADC = (Vref3.3V / Vref5V) * 1023

setpoint = 23                        # change this temperature setpoint not the next line
holding_duty_cycle = 0              # default duty cycle while other processes are running

v1_fluid_value_low = 0.945          # values that fluid sensor is looking for when sensing fluid.
v1_fluid_value_high = 0.962         # values are out of 1.0
...
try:
    ...
    with open('./BioSamplr/Sample_Logs/Process_%s/Process_%s_Sample_Log.csv' % (process, process), 'w', newline='') as file:
        ...
        writer = csv.writer(file)
        writer.writerow(["Sample Number", "Sample Time", "Cooling Block Temperature (C)", "Sample Interval = %s" % (sample_interval), "Successful?"])
        writer.writerow(["", "", "", "", ""])
