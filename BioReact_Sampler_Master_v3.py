...
def makeDir():
    path = 'OpenAutoSample/Sample_Logs/Process_%s' % (process)
...
try:
    ...
    with open('./OpenAutoSample/Sample_Logs/Process_%s/Process_%s_Temp_Log.csv' % (process, process), 'w', newline='') as file:
        temp_log = './OpenAutoSample/Sample_Logs/Process_%s/Process_%s_Temp_Log.csv' % (process, process)
        writer = csv.writer(file)
        writer.writerow(["Time", "Probe #1 - Temp (C)", "Probe #2 - Temp (C)"])   

    # Begin Sample Log.
    with open('./OpenAutoSample/Sample_Logs/Process_%s/Process_%s_Sample_Log.csv' % (process, process), 'w', newline='') as file:
        sample_log = './OpenAutoSample/Sample_Logs/Process_%s/Process_%s_Sample_Log.csv' % (process, process)
        writer = csv.writer(file)
